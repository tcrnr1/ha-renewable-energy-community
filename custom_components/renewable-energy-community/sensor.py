"""Sensor platform for Renewable Energy Community."""
...
class RenewableEnergyCommunitySensor(SensorEntity):
    """Renewable Energy Community Sensor class."""
    ...
    async def import_report(self, path: str) -> None:
        ...
        start = None
        csv_data_value_key = self.csv_column
...
def validate_hour_block(hour_block: list) -> bool:
    ...
    for index, record in enumerate(hour_block):
        ...
        if dt.hour != first_hour:
            return False

        if dt.date() != first_date:
            return False

        if dt.minute != expected_minutes[index]:
            return False
...
class RenewableEnergyCommunitySensor(SensorEntity):
    ...
    def __init__(
        ...
        self.csv_column = sensor_config["column"]

        self._attr_name = f"{_name} {sensor_config['name']}"
